...
exercice_tab::exercice_tab(bdd_note & bdd, tab_index pair, QWidget * parent)
    ...
    m_type_exo = delegate->create_sub_node(m_model->index(mps::model_base::node_index(),0,mps::model_base::Sub_Node_Cible,
                                                          exercice_edit_model::Type_Position));
    m_type_exo->update_data(mps::model_base::All_Role);
    connect(m_model,&exercice_edit_model::data_changed,this,[this](const mps::model_base::node_index &index, flag role){
        if(index.cible() == exercice_edit_model::Type_Cible && index.is_root())
...
void exercice_tab::update_type() {
    auto types = m_model->data(m_model->index(mps::model_base::node_index(),0,exercice_edit_model::Type_Cible),
                               mps::model_base::List_Of_Values);
