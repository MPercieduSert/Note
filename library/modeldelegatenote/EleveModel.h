 ...
 */
class AbstractEleveModel : public modelMPS::AbstractTableUniqueModel {
    Q_OBJECT
public:
    enum Column {Nom,
                Prenom,
    ...
    AbstractEleveModel(BddNote & bdd, QObject * parent = nullptr)
        : AbstractTableUniqueModel(bdd,parent) {}
...
public:
    ...
    EleveModel(BddNote & bdd, QObject * parent = nullptr)
        ...
        for (auto iter =m_columns.begin();iter != std::prev(m_columns.end()); ++iter)
            static_cast<EntityColumn<Eleve> &>(**iter).setFlags(Qt::ItemIsEnabled|Qt::ItemIsSelectable|Qt::ItemIsEditable);
        static_cast<EntityColumn<Eleve>&>(*m_columns[Sexe])
                .setFlags(Qt::ItemIsEnabled|Qt::ItemIsSelectable|Qt::ItemIsUserCheckable);
    }

    //! Teste l'égalité entre deux ligne
    bool egalRow(szt row1, szt row2) const override {
        auto & e1 =static_cast<const Eleve &>(m_data.front()[row1]);
        auto & e2 =static_cast<const Eleve &>(m_data.front()[row2]);
        return e1.nom() == e2.nom() && e1.prenom() == e2.prenom() && e1.date() == e2.date();
    }
